...
void fire( player_t* p )
{
  ...
  action_priority_list_t* ff_combustion_phase = p->get_action_priority_list( "ff_combustion_phase" );
  ...
  action_priority_list_t* ff_standard_rotation = p->get_action_priority_list( "ff_standard_rotation" );
  ...
  default_->add_action( "use_item,name=treacherous_transmitter,if=variable.time_to_combustion<10|fight_remains<25", "The War Within S1&2 On-Use items with special use timings" );
  ...
  sf_combustion_phase->add_action( "call_action_list,name=combustion_cooldowns,if=buff.combustion.remains>variable.skb_duration|fight_remains<20", "Other cooldowns that should be used with Combustion should only be used with an actual Combustion cast and not with a Sun King's Blessing proc." );
  sf_combustion_phase->add_action( "call_action_list,name=active_talents" );
  sf_combustion_phase->add_action( "flamestrike,if=buff.combustion.down&buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&buff.fury_of_the_sun_king.expiration_delay_remains=0&cooldown.combustion.remains<cast_time&active_enemies>=variable.skb_flamestrike", "If Combustion is down, precast something before activating it." );
  sf_combustion_phase->add_action( "pyroblast,if=buff.combustion.down&buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&(buff.fury_of_the_sun_king.expiration_delay_remains=0|buff.flame_accelerant.up)" );
  sf_combustion_phase->add_action( "meteor,if=buff.combustion.down&cooldown.combustion.remains<cast_time" );
  sf_combustion_phase->add_action( "fireball,if=buff.combustion.down&cooldown.combustion.remains<cast_time&active_enemies<2&!improved_scorch.active&!(talent.sun_kings_blessing&talent.flame_accelerant)" );
  sf_combustion_phase->add_action( "scorch,if=buff.combustion.down&cooldown.combustion.remains<cast_time" );
 
  sf_combustion_phase->add_action( "combustion,use_off_gcd=1,use_while_casting=1,if=hot_streak_spells_in_flight=0&buff.combustion.down&variable.time_to_combustion<=0&(action.scorch.executing&action.scorch.execute_remains<variable.combustion_cast_remains|action.fireball.executing&action.fireball.execute_remains<variable.combustion_cast_remains|action.pyroblast.executing&action.pyroblast.execute_remains<variable.combustion_cast_remains|action.flamestrike.executing&action.flamestrike.execute_remains<variable.combustion_cast_remains|!talent.isothermic_core&action.meteor.in_flight&action.meteor.in_flight_remains<variable.combustion_cast_remains|talent.isothermic_core&action.meteor.in_flight)", "Combustion should be used when the precast is almost finished or when Meteor is about to land." );
  sf_combustion_phase->add_action( "variable,name=TA_combust,value=cooldown.combustion.remains<10&buff.combustion.up", "Variable to determine which Fire Blast conditions are used." );
  sf_combustion_phase->add_action( "phoenix_flames,if=talent.phoenix_reborn&buff.heating_up.react&!buff.hot_streak.react&buff.born_of_flame.up", "Spend empowered Phoenix Flames at highest priority in combustion when talented into Sunfury." );
  sf_combustion_phase->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=gcd.remains<gcd.max&(!variable.TA_combust|talent.sun_kings_blessing)&!variable.fire_blast_pooling&(!improved_scorch.active|action.scorch.executing|debuff.improved_scorch.remains>4*gcd.max)&(buff.fury_of_the_sun_king.down|action.pyroblast.executing)&buff.combustion.up&!buff.hot_streak.react&hot_streak_spells_in_flight+buff.heating_up.react*(gcd.remains>0)<2", "Fire Blast usage for a standard combustion" );
  ...
  sf_combustion_phase->add_action( "cancel_buff,name=hyperthermia,if=buff.fury_of_the_sun_king.react", "Cancelaura HT if SKB is ready" );
  sf_combustion_phase->add_action( "flamestrike,if=(buff.hot_streak.react&active_enemies>=variable.combustion_flamestrike)|(buff.hyperthermia.react&active_enemies>=variable.combustion_flamestrike-talent.hyperthermia)", "Spend Hot Streaks during Combustion at high priority." );
  sf_combustion_phase->add_action( "pyroblast,if=buff.hyperthermia.react" );
  sf_combustion_phase->add_action( "pyroblast,if=buff.hot_streak.react&buff.combustion.up" );
  sf_combustion_phase->add_action( "pyroblast,if=prev_gcd.1.scorch&buff.heating_up.react&active_enemies<variable.combustion_flamestrike&buff.combustion.up" );
  sf_combustion_phase->add_action( "scorch,if=talent.sun_kings_blessing&improved_scorch.active&debuff.improved_scorch.remains<3*gcd.max" );
  ...
  sf_combustion_phase->add_action( "pyroblast,if=buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&buff.fury_of_the_sun_king.expiration_delay_remains=0" );
  sf_combustion_phase->add_action( "phoenix_flames,if=talent.phoenix_reborn&buff.heating_up.react+hot_streak_spells_in_flight<2&buff.born_of_flame.up" );
  sf_combustion_phase->add_action( "scorch,if=improved_scorch.active&(debuff.improved_scorch.remains<4*gcd.max)&active_enemies<variable.combustion_flamestrike" );
  sf_combustion_phase->add_action( "scorch,if=buff.heat_shimmer.react&(talent.scald|talent.improved_scorch)&active_enemies<variable.combustion_flamestrike" );
  sf_combustion_phase->add_action( "phoenix_flames,if=(!talent.call_of_the_sun_king&travel_time<buff.combustion.remains|(talent.call_of_the_sun_king&buff.combustion.remains<4|buff.sun_kings_blessing.stack<8))&buff.heating_up.react+hot_streak_spells_in_flight<2", "Use Phoenix Flames and Scorch in Combustion to help generate Hot Streaks when Fire Blasts are not available or need to be conserved." );
  sf_combustion_phase->add_action( "scorch,if=buff.combustion.remains>cast_time&cast_time>=gcd.max" );
  sf_combustion_phase->add_action( "fireball" );

  ff_combustion_phase->add_action( "call_action_list,name=combustion_cooldowns,if=buff.combustion.remains>variable.skb_duration|fight_remains<20" );
  ff_combustion_phase->add_action( "call_action_list,name=active_talents" );
  ff_combustion_phase->add_action( "flamestrike,if=buff.combustion.down&buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&buff.fury_of_the_sun_king.expiration_delay_remains=0&cooldown.combustion.remains<cast_time&active_enemies>=variable.skb_flamestrike", "If Combustion is down, precast something before activating it." );
  ff_combustion_phase->add_action( "meteor,if=buff.excess_fire.stack<2&(cooldown.combustion.remains<3|buff.combustion.remains>2)" );
  ff_combustion_phase->add_action( "pyroblast,if=buff.combustion.down&buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&(buff.fury_of_the_sun_king.expiration_delay_remains=0|buff.flame_accelerant.up)" );
  ff_combustion_phase->add_action( "fireball,if=prev_gcd.1.meteor&!buff.frostfire_empowerment.react&buff.combustion.down" );
  ff_combustion_phase->add_action( "scorch,if=buff.combustion.down&scorch_execute.active" );
  ff_combustion_phase->add_action( "fireball,if=buff.combustion.down&buff.hyperthermia.down" );
  ff_combustion_phase->add_action( "pyroblast,if=buff.combustion.down&buff.hyperthermia.up" );
  ff_combustion_phase->add_action( "combustion,use_off_gcd=1,use_while_casting=1,if=buff.combustion.remains<gcd.max&(action.scorch.executing&action.scorch.execute_remains<0.3|action.fireball.executing&action.fireball.execute_remains<0.3|action.pyroblast.executing&action.pyroblast.execute_remains<0.3|action.meteor.in_flight&buff.bloodlust.down)" );
  ff_combustion_phase->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=gcd.remains<gcd.max&&(buff.fury_of_the_sun_king.down|action.pyroblast.executing|action.flamestrike.executing)&!buff.hot_streak.react&hot_streak_spells_in_flight+buff.heating_up.react*(gcd.remains>0)<2&buff.combustion.up" );
  ff_combustion_phase->add_action( "cancel_buff,name=hyperthermia,if=buff.fury_of_the_sun_king.react", "Cancelaura HT if SKB is ready" );
  ff_combustion_phase->add_action( "flamestrike,if=active_enemies>=variable.combustion_flamestrike&(buff.hot_streak.react|buff.hyperthermia.react)", "Spend Hot Streaks inside Combustion" );
  ff_combustion_phase->add_action( "flamestrike,if=active_enemies>=variable.combustion_flamestrike&prev_gcd.1.scorch&buff.heating_up.react" );
  ff_combustion_phase->add_action( "pyroblast,if=buff.hot_streak.react|buff.hyperthermia.react" );
  ff_combustion_phase->add_action( "pyroblast,if=prev_gcd.1.scorch&buff.heating_up.react" );
  ff_combustion_phase->add_action( "scorch,if=!talent.unleashed_inferno&improved_scorch.active&debuff.improved_scorch.remains<3*gcd.max", "Maintain Improved Scorch inside combustion at a high priority for SKB builds." );
  ff_combustion_phase->add_action( "scorch,if=buff.heat_shimmer.react&(talent.scald|talent.improved_scorch)&active_enemies<variable.combustion_flamestrike", "Spend Heat Shimmer procs inside combustion" );
  ff_combustion_phase->add_action( "flamestrike,if=buff.fury_of_the_sun_king.up&active_enemies>=variable.skb_flamestrike", "Spend Fury of the Sun King procs inside of a combustion" );
  ff_combustion_phase->add_action( "pyroblast,if=buff.fury_of_the_sun_king.up" );
  ff_combustion_phase->add_action( "phoenix_flames,if=buff.excess_frost.react" );
  ff_combustion_phase->add_action( "fireball,if=buff.frostfire_empowerment.react" );
  ff_combustion_phase->add_action( "phoenix_flames,if=!hot_streak_spells_in_flight" );
  ff_combustion_phase->add_action( "fireball,if=time-action.pyroblast.last_used<0.15&buff.flame_accelerant.up&buff.frostfire_empowerment.down&!scorch_execute.active", "attempt to abuse lingering FA after a SKB hardcast" );
  ...
  sf_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.hot_streak_flamestrike&(buff.hot_streak.react|buff.hyperthermia.react)" );
  sf_standard_rotation->add_action( "fireball,if=buff.hot_streak.up&buff.hyperthermia.down&!cooldown.shifting_power.ready&cooldown.phoenix_flames.charges<1&!scorch_execute.active&!prev_gcd.1.fireball,line_cd=2*gcd.max", "When resources are low, fish for Hot Streaks." );
  sf_standard_rotation->add_action( "pyroblast,if=(buff.hyperthermia.react|buff.hot_streak.react&(buff.hot_streak.remains<action.fireball.execute_time)|buff.hot_streak.react&(hot_streak_spells_in_flight|firestarter.active|talent.call_of_the_sun_king&action.phoenix_flames.charges)|buff.hot_streak.react&scorch_execute.active)" );
  sf_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.skb_flamestrike&buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.expiration_delay_remains=0" );
  sf_standard_rotation->add_action( "scorch,if=improved_scorch.active&((talent.unleashed_inferno&debuff.improved_scorch.remains<action.pyroblast.cast_time+5*gcd.max)|(talent.sun_kings_blessing&debuff.improved_scorch.remains<4*gcd.max))&buff.fury_of_the_sun_king.up&!action.scorch.in_flight" );
  sf_standard_rotation->add_action( "pyroblast,if=buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.expiration_delay_remains=0" );
  sf_standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=!firestarter.active&(!variable.fire_blast_pooling|talent.spontaneous_combustion)&buff.fury_of_the_sun_king.down&(((action.fireball.executing&(action.fireball.execute_remains<0.5|!talent.hyperthermia)|action.pyroblast.executing&(action.pyroblast.execute_remains<0.5))&buff.heating_up.react)|(scorch_execute.active&(!improved_scorch.active|debuff.improved_scorch.stack=debuff.improved_scorch.max_stack|full_recharge_time<3)&(buff.heating_up.react&!action.scorch.executing|!buff.hot_streak.react&!buff.heating_up.react&action.scorch.executing&!hot_streak_spells_in_flight)))", "During the standard rotation, only use Fire Blasts when they are not being pooled for Combustion. Use Fire Blast either during a Fireball/Pyroblast cast when Heating Up is active or during execute with Searing Touch." );
  ...
  sf_standard_rotation->add_action( "pyroblast,if=prev_gcd.1.scorch&buff.heating_up.react&scorch_execute.active&active_enemies<variable.hot_streak_flamestrike" );
  sf_standard_rotation->add_action( "scorch,if=improved_scorch.active&debuff.improved_scorch.remains<4*gcd.max" );
  sf_standard_rotation->add_action( "fireball,if=buff.frostfire_empowerment.up&!buff.hot_streak.react&!buff.excess_frost.up" );
  sf_standard_rotation->add_action( "scorch,if=buff.heat_shimmer.react&(talent.scald|talent.improved_scorch)&active_enemies<variable.combustion_flamestrike" );
  sf_standard_rotation->add_action( "phoenix_flames,if=!buff.hot_streak.up&(hot_streak_spells_in_flight<1&(!prev_gcd.1.fireball|(buff.heating_up.down&buff.hot_streak.down)))|(hot_streak_spells_in_flight<2&buff.born_of_flame.react)" );
  sf_standard_rotation->add_action( "call_action_list,name=active_talents" );
  sf_standard_rotation->add_action( "dragons_breath,if=active_enemies>1&talent.alexstraszas_fury" );
  sf_standard_rotation->add_action( "scorch,if=(scorch_execute.active|buff.heat_shimmer.react)" );
  sf_standard_rotation->add_action( "arcane_explosion,if=active_enemies>=variable.arcane_explosion&mana.pct>=variable.arcane_explosion_mana" );
  sf_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.hard_cast_flamestrike", "With enough targets, it is a gain to cast Flamestrike as filler instead of Fireball. This is currently never true up to 10t." );
  sf_standard_rotation->add_action( "fireball" );

  ff_standard_rotation->add_action( "meteor,if=talent.unleashed_inferno&buff.excess_fire.stack<2" );
  ff_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.hot_streak_flamestrike&(buff.hot_streak.react|buff.hyperthermia.react)" );
  ff_standard_rotation->add_action( "pyroblast,if=buff.hot_streak.react|buff.hyperthermia.react" );
  ff_standard_rotation->add_action( "scorch,if=improved_scorch.active&debuff.improved_scorch.remains<3*gcd.max&!prev_gcd.1.scorch" );
  ff_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.skb_flamestrike&buff.fury_of_the_sun_king.react" );
  ff_standard_rotation->add_action( "pyroblast,if=buff.fury_of_the_sun_king.react" );
  ff_standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=(buff.fury_of_the_sun_king.down&action.fireball.executing&action.fireball.execute_remains<0.5&buff.heating_up.react&gcd.remains<0.5)&!buff.hyperthermia.up&(talent.unleashed_inferno&variable.combustion_ready_time>8|talent.sun_kings_blessing)", "Filler fireblast lines with rudimentary pooling rules for Unleashed Inferno builds" );
  ff_standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=scorch_execute.active&(debuff.improved_scorch.stack=2|full_recharge_time<3)&(!buff.hot_streak.react&!buff.heating_up.react&action.scorch.executing)&gcd.remains<0.5&(talent.unleashed_inferno&variable.combustion_ready_time>8|talent.sun_kings_blessing)" );
  ff_standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=(charges_fractional>2.5&buff.heating_up.react&gcd.remains<0.5)&(talent.unleashed_inferno&variable.combustion_ready_time>8|talent.sun_kings_blessing)" );
  ff_standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=buff.hyperthermia.up&charges_fractional>1.5&buff.heating_up.react" );
  ff_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.hot_streak_flamestrike&prev_gcd.1.scorch&buff.heating_up.react" );
  ff_standard_rotation->add_action( "pyroblast,if=prev_gcd.1.scorch&buff.heating_up.react" );
  ff_standard_rotation->add_action( "phoenix_flames,if=buff.excess_frost.react" );
  ff_standard_rotation->add_action( "fireball,if=buff.frostfire_empowerment.react" );
  ff_standard_rotation->add_action( "phoenix_flames" );
  ff_standard_rotation->add_action( "scorch,if=buff.heat_shimmer.react&(talent.scald|talent.improved_scorch)&active_enemies<variable.combustion_flamestrike", "Spend Heat Shimmer procs" );
  ff_standard_rotation->add_action( "scorch,if=(talent.sun_kings_blessing&scorch_execute.active)|(improved_scorch.active&debuff.improved_scorch.remains<3*gcd.max)" );
  ff_standard_rotation->add_action( "arcane_explosion,if=active_enemies>=variable.arcane_explosion&mana.pct>=variable.arcane_explosion_mana" );
  ff_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.hard_cast_flamestrike", "With enough targets, it is a gain to cast Flamestrike as filler instead of Fireball. This is currently never true" );
  ff_standard_rotation->add_action( "fireball" );
}
//fire_apl_end
